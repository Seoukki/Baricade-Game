...
export default function Lobby() {
  ...
  useEffect(() => {
    ...
    // First fetch to get room id, then subscribe with proper filter
    (async () => {
      await fetchData();
      // room state is set after fetchData resolves — subscribe reactively
    })();
    ...
    const channel = supabase
      ...
      .on("postgres_changes",
        ...
        (payload) => {
          // Redirect immediately on status change without waiting for full refetch
  ...
  async function startGame() {
    ...
    try {
      ...
      const { error: gse } = await supabase
        .from("game_states")
        .insert({
          room_id:      room.id,
          current_turn: "red",
      ...
      console.error(err);
      setError("Gagal memulai game. Coba lagi.");
      setStarting(false);
    }
  }
    ...
    return (
      <div className="min-h-screen grid-bg flex items-center justify-center">
        <div className="card p-8 flex flex-col items-center gap-4">
          <div className="spinner !border-[var(--blue)] !border-t-transparent !w-8 !h-8" style={{ borderColor: "#3D6BD9", borderTopColor: "transparent" }} />
  ...
  return (
    <>
      <Head>
        <title>Lobby {code} — Barricade</title>
      </Head>
      ...
      <div className="min-h-screen grid-bg flex flex-col items-center justify-center px-4 py-10">
        ...
        <div className="text-center mb-8 animate-fade-up">
          ...
          <h1 className="font-display text-4xl font-black text-[var(--text)] leading-tight">
            Ruang Tunggu
          </h1>
          <p className="text-[var(--muted)] text-sm mt-1">
            Bagikan kode ke temanmu untuk bergabung
          </p>
        </div>
        ...
        <div className="w-full max-w-sm space-y-4 animate-fade-up-delay">
          ...
          <div className="card p-5">
            <p className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-3 text-center">
              Kode Room
            </p>
            ...
            {copied && (
              <p className="text-center text-xs text-green-500 mt-2 animate-fade-in">
                ✓ Kode tersalin!
              </p>
            )}
          </div>
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3">
              <PlayerSlot
                ...
                color="#D94F3D"
                player={redPlayer}
                isMe={redPlayer?.session_id === sessionId}
                emoji="🔴"
                bgColor="rgba(217,79,61,0.08)"
              ...
              <PlayerSlot
                ...
                color="#3D6BD9"
                player={bluePlayer}
                isMe={bluePlayer?.session_id === sessionId}
                emoji="🔵"
                bgColor="rgba(61,107,217,0.08)"
                borderColor="rgba(61,107,217,0.2)"
              />
            </div>
          </div>
              ...
              <div className="text-center text-sm text-[var(--muted)] py-1">
                {allReady
                  ...
                  : players.length < 2
                    ...
                    : "Tandai dirimu siap untuk melanjutkan"}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-center gap-6 py-2">
            <StatusDot label="Tersambung" active={true} color="#22c55e" />
            <StatusDot label={`${players.length}/2 Pemain`} active={players.length === 2} color="#3D6BD9" />
            <StatusDot label="Semua Siap" active={allReady} color="#D94F3D" />
          </div>

          {error && (
            <div className="px-4 py-3 rounded-xl bg-red-50 border border-red-100 text-red-600 text-sm text-center animate-fade-in">
              {error}
            </div>
          )}
        </div>
...
function PlayerSlot({ label, color, player, isMe, emoji, bgColor, borderColor }) {
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {player ? (
          <p className="text-sm font-medium text-[var(--text)] truncate">
            {player.player_name}
            {isMe && <span className="ml-1.5 text-[var(--muted)] text-xs">(Kamu)</span>}
          </p>
        ) : (
      ...
      {player && (
        <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${player.is_ready ? "bg-green-500" : "bg-[var(--border)]"}`}>
          {player.is_ready ? "✓" : "?"}
        </div>
      )}
      {!player && (
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs text-[var(--muted)]">
      <span
        ...
        style={{ background: active ? color : "#D4CFC6" }}
      />
      {label}
    </div>
  );
