...
import { MAX_BARRICADES, generateSessionId } from "../../lib/gameLogic";
...
export default function Lobby() {
  ...
  useEffect(() => {
    if (!code) return;
    fetchData();
    const channel = supabase.channel(`lobby:${code}`, { config: { presence: { key: sessionId } } })
      ...
      .subscribe(async status => {
        if (status === "SUBSCRIBED") await channel.track({ session_id: sessionId, page: "lobby" });
      });
    channelRef.current = channel;
    return () => { supabase.removeChannel(channel); };
  }, [code, fetchData, router, sessionId]);
  ...
  async function startGame() {
    ...
    if (players.length < 2) { setError(tr("waiting_second")); return; }
    ...
    } catch (e) { console.error(e); setError("Gagal memulai."); setStarting(false); }
  }
  ...
  const qrUrl = code && typeof window !== "undefined"
    ? `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(`${window.location.origin}/lobby/${code}`)}&color=2B2825&bgcolor=F7F2EA&margin=8`
    : "";
  ...
  return (
    <>
      ...
      <div className="min-h-screen grid-bg flex flex-col items-center px-4 py-10 pb-20">

        {/* Back */}
        <div className="w-full max-w-sm mb-6 mt-8">
        ...
        <div className="text-center mb-6 w-full max-w-sm">
          <div className="flex items-center justify-center gap-2 mb-1">
            <h1 className="font-display text-3xl font-black text-[var(--text)]">{tr("waiting_room")}</h1>
            <span className="text-xs font-semibold px-2 py-1 rounded-full"
              style={isPrivate
                ? {background:"rgba(217,79,61,0.1)",color:"var(--red)"}
                : {background:"rgba(61,107,217,0.1)",color:"var(--blue)"}}>
        ...
        <div className="w-full max-w-sm space-y-4">
          ...
          <div className="card p-5">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex flex-col gap-1.5">
                <button onClick={copyCode} className="btn btn-ghost px-3 py-2.5 !rounded-xl" title="Copy">
                  {copied
                ...
                <button onClick={() => setShowQR(!showQR)}
                  className="btn btn-ghost px-3 py-2 !rounded-xl text-xs font-bold">QR</button>
              </div>
            </div>
            {copied && <p className="text-center text-xs text-green-500 mb-2 animate-fade-in">{tr("copied")}</p>}

            {showQR && (
              <div className="mt-3 text-center border-t border-[var(--border)] pt-4 animate-fade-in">
                <p className="text-xs text-[var(--muted)] mb-3">{tr("qr_hint")}</p>
                {qrUrl && <img src={qrUrl} alt="QR" className="mx-auto rounded-xl border border-[var(--border)]" width={180} height={180}/>}
                <button onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/lobby/${code}`);
                  setCopied(true); setTimeout(() => setCopied(false), 2000);
                }} className="btn btn-ghost text-xs mt-3 px-4 py-2">🔗 Copy Link</button>
              </div>
            )}
          ...
          {/* Players */}
          <div className="card p-5">
            <p className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-4">{tr("player_count")} ({players.length}/2)</p>
              ...
              {[
                ...
                <div key={s.label} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {s.player
                      ? <p className="text-sm font-medium text-[var(--text)] truncate">{s.player.player_name}{s.isMe&&<span className="ml-1.5 text-[var(--muted)] text-xs">{tr("you")}</span>}</p>
                      : <p className="text-sm text-[var(--muted)] italic">{tr("waiting_player")}</p>}
                  </div>
                  {s.player
                    ? <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${s.player.is_ready?"bg-green-500":"bg-[var(--border)]"}`}>{s.player.is_ready?"✓":"?"}</div>
                    : <div className="w-2 h-2 rounded-full bg-[var(--border)] animate-pulse flex-shrink-0"/>}
                </div>
              ))}
            </div>
            <div className="mt-3 px-3 py-2.5 rounded-xl bg-[var(--cream)] border border-[var(--border)] text-xs text-center text-[var(--muted)]">
          ...
          <div className="card p-5 space-y-3">
            ...
            {isHost ? (
              <button className={`btn w-full text-base py-3.5 ${allReady?"btn-blue":"btn-ghost"}`}
                onClick={startGame} disabled={!allReady||starting||players.length<2}>
                {starting ? <span className="spinner"/>
                  ...
                  {allReady?tr("start_game"):players.length<2?tr("waiting_second"):tr("waiting_all_ready")}</>}
              </button>
            ) : (
              <p className="text-center text-sm text-[var(--muted)] py-1">
          ...
          <div className="flex items-center justify-center gap-6 py-1">
            {[[tr("connected"),true,"#22c55e"],[`${players.length}/2`,players.length===2,"#3D6BD9"],[tr("all_ready"),allReady,"#D94F3D"]].map(([lbl,active,col])=>(
              <div key={lbl} className="flex items-center gap-1.5 text-xs text-[var(--muted)]">
                <span className="w-2 h-2 rounded-full" style={{background:active?col:"#D4CFC6"}}/>
                {lbl}
              </div>
            ))}
          </div>
